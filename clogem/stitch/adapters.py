...
def try_stitch_adapters(stitch_prompt: str) -> StitchResult:
    ...
    1. CLOGEM_STITCH_CLI — subprocess (stdin or temp file)
    2. CLOGEM_STITCH_MCP — MCP stdio client to `npx stitch-mcp` (on by default; set CLOGEM_STITCH_MCP=0 to disable)
    3. CLOGEM_STITCH_HTTP_URL — HTTP POST JSON { "prompt": "..." }
    4. CLOGEM_STITCH_BROWSER — optional stub (disabled unless set; not implemented)
    ...
    return StitchResult.manual(
        "No direct Stitch integration succeeded; use manual export (see clogem instructions)."
    )
...
def _try_mcp_adapter(prompt: str) -> StitchResult:
    ...
    if not stitch_mcp_enabled():
        return StitchResult.unavailable("mcp", "disabled (set CLOGEM_STITCH_MCP=1 to enable)")
...
def _try_cli_adapter(prompt: str) -> StitchResult:
    raw_cli = (os.environ.get("CLOGEM_STITCH_CLI") or "").strip()
    if not raw_cli:
        return StitchResult.unavailable("cli", "CLOGEM_STITCH_CLI not set")

    argv = shlex_split(raw_cli)
    extra = os.environ.get("CLOGEM_STITCH_CLI_ARGS", "")
    if extra.strip():
    ...
    use_stdin = (os.environ.get("CLOGEM_STITCH_CLI_STDIN", "1").strip().lower() not in ("0", "false", "no"))
    tmp_path: Optional[str] = None
    try:
        if not use_stdin:
            fd, tmp_path = tempfile.mkstemp(suffix=".txt", prefix="clogem-stitch-")
            os.close(fd)
...
def _timeout_sec() -> int:
    try:
        return max(30, int(os.environ.get("CLOGEM_STITCH_TIMEOUT_SEC", "300")))
    except ValueError:
        return 300
...
def _try_http_adapter(prompt: str) -> StitchResult:
    url = (os.environ.get("CLOGEM_STITCH_HTTP_URL") or "").strip()
    if not url:
        return StitchResult.unavailable("http", "CLOGEM_STITCH_HTTP_URL not set")

    payload = {"prompt": prompt}
    raw_body = (os.environ.get("CLOGEM_STITCH_HTTP_BODY") or "").strip()
    if raw_body:
        try:
    ...
    token = (os.environ.get("CLOGEM_STITCH_HTTP_TOKEN") or "").strip()
    if token:
        headers["Authorization"] = f"Bearer {token}"

    method = (os.environ.get("CLOGEM_STITCH_HTTP_METHOD") or "POST").upper()
...
def _try_browser_adapter(prompt: str) -> StitchResult:
    flag = (os.environ.get("CLOGEM_STITCH_BROWSER") or "").strip().lower()
    if flag not in ("1", "yes", "true", "on"):
        return StitchResult.unavailable("browser", "disabled (set CLOGEM_STITCH_BROWSER=1 to enable)")
    return StitchResult.unavailable(
        "browser",
...
def _stitch_retry_attempts() -> int:
    try:
        return max(1, int(os.environ.get("CLOGEM_STITCH_RETRY_ATTEMPTS", "2")))
    except ValueError:
        return 2
